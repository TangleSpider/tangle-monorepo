...
contract Fractal {
    ...
    using Brown for Brown.Data;
    Brown.Data _brown;
    using Trades for Trades.Data;
    Trades.Data _trades;

    function propose(
        Proposal calldata proposal,
        bytes calldata goods
    ) external payable
    {
        _trades.propose(proposal);
    }

    function accept(Acceptance calldata acceptance) external payable
    {
        _trades.accept(acceptance);
    }

    function moil(Work calldata work) external
    {
        _brown.push(work);
    }
...
library Trades {
    ...
    struct Data
    {
        mapping(bytes32 => bytes32) trades;
    }

    function propose(Data storage self, Proposal calldata proposal) internal
    {
        require(
            msg.value ==
            proposal.fee + (proposal.party == 0 ? proposal.cost : 0)
        );
        bytes32 hash = keccak256(abi.encode(proposal));
        require(self.trades[hash] == bytes32(0));
        self.trades[hash] = bytes32(uint(1));
    }

    function accept(Data storage self, Acceptance calldata acceptance) internal
    {
        Proposal memory proposal = acceptance.proposal;
        require(
            msg.value ==
            acceptance.fee + (proposal.party == 0 ? proposal.cost : 0)
        );
        require(acceptance.party != proposal.party);
        require(acceptance.party == 0 || proposal.party == 0);
        bytes32 hash = keccak256(abi.encode(proposal));
        require(self.trades[hash] == bytes32(uint(1)));
        self.trades[hash] = keccak256(abi.encode(acceptance));
    }
...
library Brown {
    ...
    function push(Data storage self, Work calldata work) internal
    {
        self.map[ruler(self.index++)] = keccak256(abi.encode(work));
    }
    ...
    function ruler(uint x) internal pure returns (uint8)
    {
        return log2(x ^ x + 1);
    }
