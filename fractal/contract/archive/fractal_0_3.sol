...
contract Fractal {
    ...
    function init(
        ...
        uint minW
                 ...
                 /*seller*/          /*buyer*/
        require((msg.value == 0) != (msg.value == cost + tax));
        trades.push(Trade(
            /*status*/  0,
            /*start*/   0,
            /*delay*/   delay,
            /*timeout*/ timeout,
            /*buyer*/   msg.value == cost + tax ? msg.sender : address(0),
            /*seller*/  msg.value == 0 ? msg.sender : address(0),
            /*cost*/    cost,
            /*tax*/     tax,
            /*product*/ product,
            /*wMin*/    minW,
    ...
    function accept(uint id) external payable {
        ...
        require(
            ...
            /*buyer*/
            (msg.value == trade.cost + trade.tax && trade.buyer == address(0))
        );
    ...
    function execute(uint id) external {
        ...
        require(
            ...
            trade.w[0] + trade.w[1] >= trade.wMin &&
            trade.w[0] != trade.w[1]
        );
        trade.w[0] > trade.w[1] ?
            payable(trade.seller).transfer(trade.cost):
            payable(trade.buyer).transfer(trade.cost);
        trade.status = 2;
    }
    ...
    function refund(uint id) external {
        ...
        payable(trade.buyer).transfer(
            trade.cost + (trade.w[0] + trade.w[1] > 0 ? 0 : trade.tax)
        );
        trade.status = 2;
    }
    ...
    function redeem(uint id) external {
        ...
        payable(msg.sender).transfer(trade.tax * wI / wT);
