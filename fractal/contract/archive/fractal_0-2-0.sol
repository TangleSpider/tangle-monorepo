...
contract Fractal {
    ...
    function propose(Proposal calldata proposal) external payable
    {
        trades.propose(proposal);
    }

    function accept(Proposal calldata proposal) external payable
    {
        trades.accept(proposal);
    }

    function submitWork(bytes32[] calldata acknowledgedWorks, bytes32[] calldata relevantTrades, bool[] calldata decisions, uint nonce) external
    {
        _brown.push(keccak256(abi.encode(acknowledgedWorks, relevantTrades, decisions, nonce)));
    }
...
library Trade {
    ...
    function propose(Data storage self, Proposal calldata proposal) internal {
        ...
        self.trades[hash] = keccak256(abi.encode(proposal));
    }

    function accept(Data storage self, Proposal calldata proposal) internal {
        require(msg.value >= proposal.cost);
    }
...
library Brown {
    ...
    function push(Self storage self, bytes32 hash) internal {
        self.map[ruler(self.index++)] = hash;
    }
