...
contract Meta {
    ...
    function buyToken(address token) payable public {
        ...
        payable(offer.seller).transfer(msg.value * (taxConstant - 1) / taxConstant);
        payable(feeTo).transfer(msg.value * 1 / taxConstant);
    ...
    function acceptBidForToken(address token, uint minPrice) public {
        ...
        payable(seller).transfer(bid.value * (taxConstant - 1) / taxConstant);
        payable(feeTo).transfer(bid.value * 1 / taxConstant);
