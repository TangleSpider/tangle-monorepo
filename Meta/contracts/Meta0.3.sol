...
contract Meta {
    ...
    function buyToken(address token) payable public {
        ...
        tokenApprovals[token] = address(0);
        if (balanceOf[offer.seller] > 1 && indexOfTokenInTokensOfOwner[offer.seller][token] != balanceOf[offer.seller] - 1) {
        ...
        payable(offer.seller).transfer(msg.value * 99 / 100);
        payable(feeTo).transfer(msg.value * 1 / 100);
    ...
    function acceptBidForToken(address token, uint minPrice) public {
        ...
        payable(seller).transfer(bid.value * 99 / 100);
        payable(feeTo).transfer(bid.value * 1 / 100);
