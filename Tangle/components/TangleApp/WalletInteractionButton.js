import { requestAccounts } from  "../functions/requestAccounts.js";
import { getChainId } from "../functions/getChainId.js";
    ...
    methods: {
        ...
        connectWallet: async function() {
            ...
            if (account && !addressObtained && tangleAddress) {
                ...
                if (ethereum)
                    availableRewardData = await ethereum.request({ method: "eth_call", params: [{ to: tangleAddress, data: "0xfd9ff94c" + account.substr(2).padStart(64, '0') }] });
                ...
                this.$root.TNGL.state.liquidityBalance = await ethereum.request({ method: "eth_call", params: [{ to: liquidityAddress, data: "0x70a08231" + account.substr(2).padStart(64, '0') }] });
                this.$root.TNGL.state.currentlyStaked = await ethereum.request({ method: "eth_call", params: [{ to: tangleAddress, data: "0xf6b5fc2e" + account.substr(2).padStart(64, '0') + '2'.padStart(64, '0') }] });
                this.$root.$emit("block");
                await ethereum.request({ method: "eth_subscribe", params: ["newHeads", {}]  });
                ethereum.on("message", async e => {
                    try {
                        let availableRewardData = await ethereum.request({ method: "eth_call", params: [{ to: tangleAddress, data: "0xfd9ff94c" + account.substr(2).padStart(64, '0') }] });
                        this.$root.TNGL.state.availableRewards = "0x" + availableRewardData.substr(2).match(/.{64}/g)[3];
                        this.$root.TNGL.state.availableRewardsMarketMaker = "0x" + availableRewardData.substr(2).match(/.{64}/g)[0];
                        this.$root.TNGL.state.availableRewardsDistributor = "0x" + availableRewardData.substr(2).match(/.{64}/g)[1];
                        this.$root.TNGL.state.availableRewardsStaker = "0x" + availableRewardData.substr(2).match(/.{64}/g)[2];
