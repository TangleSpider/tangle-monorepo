...
contract Info {
        ...
        internal
    {
        ...
        _reflect();
        _executePostTransferHooks(spender, receiver, value);
    }

    function addHook(SLib.PTH memory hook) internal {
        SLib.SPTH storage s = SLib.getSPTH();
    ...
    function _executePostTransferHooks(
        address sender,
        address receiver,
        uint value
    ) internal {
    ...
    {
        return SLib.getSReflector().reflects;
    }
    ...
    function minHoldAmount() external view returns (uint) {
        return SLib.getSPreTransformer().minHoldAmount;
    }
    ...
    function mintOnce(address to) external {
        ...
        require(!sBalances.minted, "mintOnce minted");
        sBalances.minted = true;
        uint totalSupply_ = SLib.getSInfo().totalSupply;
        sBalances.balances[to] = unitsToPieces(totalSupply_);
        emit SLib.Transfer(address(0), to, totalSupply_);
    }
    ...
    function infoChange(SLib.SInfo calldata s_) external {
        require(msg.sender == owner, "infoChange");
        SLib.SInfo storage sInfo = SLib.getSInfo();
        sInfo.name = s_.name;
        sInfo.symbol = s_.symbol;
        sInfo.decimals = s_.decimals;
        sInfo.totalSupply = s_.totalSupply;
        sInfo.piecesPerUnit = s_.piecesPerUnit;
    ...
    {
        SLib.SMappingIDs storage sMappingIDs = SLib.getSMappingIDs();
        string[][] memory mappings = new string[][](sMappingIDs.names.length);
        for (uint i = 0; i < sMappingIDs.names.length; i++) {
            string[] memory mapping_ = new string[](2);
            mapping_[0] = sMappingIDs.names[i];
            mapping_[1] = sMappingIDs.ids[sMappingIDs.names[i]];
            mappings[i] = mapping_;
        }
        return mappings;
    }
   ...
   function splits() external view returns (SLib.Split[] memory) {
       SLib.STaxMan storage sTaxMan = SLib.getSTaxMan();
       SLib.Split[] memory splits_ = new SLib.Split[](sTaxMan.addresses.length);
       for (uint i = 0; i < sTaxMan.addresses.length; i++) {
           splits_[i] = sTaxMan.splits[sTaxMan.addresses[i]];
       }
       return splits_;
   }
   ...
   function _availableRewards(
    ...
    /// @param name_ The name of the farm whose rewards will be returned
    ...
    function availableRewards(
        ...
        return piecesToUnits(_availableRewards(address_, name_));
    }
    ...
    /// @param _address The address whose rewards will be withdrawn
    /// @param name_ The farm the rewards will be withdrawn from
    function withdrawRewards(
        address _address,
        string memory name_
    ) external {
        _withdrawRewards(_address, name_);
    }
    ...
    function points(address address_, string memory name_) external view returns (uint) {
        SLib.SFarms storage s = SLib.getSFarms(getId("farms"));
        return s.points[address_][name_];
    }
    ...
    {
        return SLib.getSAirdropping().blacklist;
    }
    ...
    function cropDustAmount() external view returns (uint) {
        return SLib.getSAirdropping().cropDustAmount;
    }
